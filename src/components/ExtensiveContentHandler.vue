<template>
    <div id="extensive-handler">
        <div v-for="(subtitle, index) in subtitels" :key="index" class="subtitle">
            <div :id="index" class="subtitleName" @click="showText">{{index+1}}. {{subtitle}}
                <div class="btnMark">{{ pressedIndex.includes(index) ? "-": "+" }}</div>
            </div>
            <advanced-open-text v-if="pressedIndex.includes(index)" :mainNum="contentNum" :subNum="index" :hativaNum="hativaNum"></advanced-open-text>
        </div>
    </div>
</template>
...
import AdvancedOpenText from "@/components/AdvancedOpenText";
export default {
    name: "extensive-content-handler",
    ...
    computed: {
        subtitels() {
            ...
            return json.advanced[0][subtitleKey] || [];
        }
    },
    data() {
        return {
